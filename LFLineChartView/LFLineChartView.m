...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        scoreTexts = [NSMutableArray array];
...
- (void) reloadData
{
    ...
    // Remove only sublayers generated by the view, if generated previously
    /*for(int i = [self.layer.sublayers count] - 1; i >= 0; i--)
    {
        CALayer *layer = [self.layer.sublayers objectAtIndex:i];
        if([items containsObject:layer] || [lines containsObject:layer] || [texts containsObject:layer] || [scoreTexts containsObject:layer] || [middleLines containsObject:layer])
        {
...
// Score Text
// -----------------------------------------------------------------
- (CATextLayer *) scoreLayerAtIndex:(NSUInteger) index
{
    if(index > 0 && index < [scoreTexts count])
    {
        return (CATextLayer *)[scoreTexts objectAtIndex:index];
    }
    else
    {
        ...
        [scoreTexts insertObject:layer atIndex:index];
        return layer;
    }
}

- (NSDictionary *) scoreTextAttributesForScore:(NSUInteger) score
{
    CTFontRef textFont = CTFontCreateWithName(CFSTR("Helvetica"), 20.0, NULL);
    ...
    NSDictionary *textAttributes = @{(NSString *)kCTFontAttributeName: (__bridge id) textFont,
                                     (NSString *)kCTForegroundColorAttributeName: (__bridge id)[UIColor whiteColor].CGColor,
...
- (CGPoint) scoreTextPositionAtIndex:(NSUInteger) index forPosition:(CGPoint) position
{
    ...
    float circleSpacing = [self radiusAtIndex:index] + _textHeight;
    
    float y = (delta < 0) ? position.y - circleSpacing : position.y + circleSpacing;
...
/*- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    for(CAShapeLayer *layer in items)
    {
        ...
        if(CGRectContainsPoint(CGRectMake(position.x-tolerance, position.y-tolerance, tolerance*2, tolerance*2), point) && i != tempLayerIndexPressed)
        {
            // Revert previously touched
            [self revertTouchedToNormalState];
            
            // Touched
            CGFloat finalRadius     = [self radiusAtIndex:i] + 10.0;
            UIBezierPath *finalPath = [UIBezierPath bezierPathWithOvalInRect:CGRectMake(-finalRadius, -finalRadius, 2*finalRadius, 2*finalRadius)];
            
            CABasicAnimation *enlargeItem = [CABasicAnimation animationWithKeyPath:@"path"];
            enlargeItem.duration        = 0.2;
            enlargeItem.fromValue       = (id)layer.path;
            enlargeItem.toValue         = (id)finalPath.CGPath;
            enlargeItem.timingFunction  = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseInEaseOut];
            
            tempPath = [UIBezierPath bezierPathWithCGPath:layer.path];
            tempLayerIndexPressed = i;
            
            layer.path = finalPath.CGPath;
            
            [layer addAnimation:enlargeItem forKey:@"path"];
        }
        else if(i == tempLayerIndexPressed)
        {
            [self revertTouchedToNormalState];
        }
...
- (void) revertTouchedToNormalState
{
    if(tempLayerIndexPressed >= 0)
    {
        CAShapeLayer *layer = [items objectAtIndex:tempLayerIndexPressed];
        
        // Touched
        CABasicAnimation *smallItem = [CABasicAnimation animationWithKeyPath:@"path"];
        smallItem.duration        = 0.2;
        smallItem.fromValue       = (id)layer.path;
        smallItem.toValue         = (id)tempPath.CGPath;
        smallItem.timingFunction  = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseInEaseOut];
        
        tempLayerIndexPressed = -1;
        layer.path = tempPath.CGPath;
        
        [layer addAnimation:smallItem forKey:@"path"];
    }
}*/
